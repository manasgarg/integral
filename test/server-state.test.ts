...
test("[SERVER-FE2BB5CF] [CONNECTION-20778353] only runner preflight checks active model connection and Docker availability", async () => { const runner = await import("node:fs/promises").then((fs) => fs.readFile("src/runner.ts", "utf8")); const server = await import("node:fs/promises").then((fs) => fs.readFile("src/server.ts", "utf8")); const coordinator = await import("node:fs/promises").then((fs) => fs.readFile("src/coordinator.ts", "utf8")); const gateway = await import("node:fs/promises").then((fs) => fs.readFile("src/gateway.ts", "utf8")); assert.match(runner, /validateRunnerHost.*selectModel.*dockerAvailable/s); assert.match(server, /validateRunnerHost/); assert.doesNotMatch(coordinator + gateway, /dockerAvailable|selectModel/); });
