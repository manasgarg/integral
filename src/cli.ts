...
async function connectionCommand(args: string[]): Promise<number> {
  ...
  if (command === "add") { await prepareStorage(paths); const setup = args[1] ? explicitConnection(args.slice(1)) : await guidedConnection(); const entry = CATALOG.find((e) => e.name === (setup.provider ?? setup.kind)); if (!entry || !entry.auth.includes(setup.auth as never)) throw new RrError(`${setup.provider ?? setup.kind} does not support ${setup.auth} authentication`);
    ...
    if (setup.auth === "key") credential = await readCredential(has(args, "--credential-stdin"));
    else if (setup.auth === "oauth" || setup.auth === "device-code") credential = await authenticateOAuth(setup);
...
async function guidedConnection(): Promise<Connection> {
  ...
  try {
    process.stdout.write(CATALOG.map((e, i) => `${i + 1}. ${e.name} (${e.kind}; ${e.auth.join(", ")})`).join("\n") + "\n"); const selected = Number(await rl.question("Select connection: ")) - 1, entry = CATALOG[selected]; if (!entry) throw new RrError("invalid selection");
    const name = (await rl.question(`Name [${entry.name}]: `)).trim() || entry.name, defaultAuth = "defaultAuth" in entry ? entry.defaultAuth : entry.auth[0]; const chosenAuth = (await rl.question(`Authentication (${entry.auth.join("/")}) [${defaultAuth}]: `)).trim() || defaultAuth; const args = [entry.name, "--name", name, "--auth", chosenAuth];
    if (entry.kind !== "model") { args.push("--url", await rl.question("URL: ")); const path = (await rl.question("Path prefix [URL path]: ")).trim(); if (path) args.push("--path-prefix", path); }
    if (entry.kind !== "model" && (chosenAuth === "oauth" || chosenAuth === "device-code")) { args.push("--authorization-url", await rl.question("Authorization URL: "), "--token-url", await rl.question("Token URL: "), "--client-id", await rl.question("Client ID: ")); if (chosenAuth === "device-code") args.push("--device-authorization-url", await rl.question("Device authorization URL: ")); }
    if (entry.kind === "mcp") { const transport = (await rl.question("Transport (streamable-http/sse) [streamable-http]: ")).trim(); if (transport) args.push("--transport", transport); }
    return explicitConnection(args);
  } finally { rl.close(); }
}
async function readCredential(fromStdin: boolean): Promise<string> {
  if (fromStdin) { const rl = createInterface({ input, output }); try { const value = (await rl.question("")).trim(); if (!value) throw new RrError("credential must not be empty"); return value; } finally { rl.close(); } }
  if (!input.isTTY) throw new RrError("credential input requires a terminal or --credential-stdin"); process.stdout.write("Credential (hidden; stored outside configuration): ");
  const value = await new Promise<string>((resolve, reject) => { let text = ""; input.setRawMode(true); input.resume(); const data = (chunk: Buffer) => { for (const byte of chunk) { if (byte === 3) { cleanup(); reject(new RrError("credential entry cancelled")); return; } if (byte === 10 || byte === 13) { cleanup(); process.stdout.write("\n"); resolve(text); return; } if (byte === 127 || byte === 8) text = text.slice(0, -1); else if (byte >= 32) text += String.fromCharCode(byte); } }; const cleanup = () => { input.off("data", data); input.setRawMode(false); input.pause(); }; input.on("data", data); });
  if (!value.trim()) throw new RrError("credential must not be empty"); return value.trim();
}
async function authenticateOAuth(connection: Connection): Promise<string> { if (!input.isTTY) throw new RrError("OAuth setup requires an interactive terminal"); const rl = createInterface({ input, output }); const ui = { show: (message: string) => process.stdout.write(`${message}\n`), prompt: (message: string) => rl.question(message) }; try { return connection.kind === "model" ? await runModelOAuth(connection.provider!, connection.auth as "oauth" | "device-code", ui) : await runGenericOAuth(connection, ui); } finally { rl.close(); } }
async function verifySetup(connection: Connection, credential?: string): Promise<void> { if (!connection.url) return; const headers: Record<string, string> = {}, effective = credential ? oauthAccess(credential) ?? credential : undefined; if (connection.auth !== "none") headers[connection.header ?? "Authorization"] = `${connection.scheme ?? "Bearer"} ${effective}`; const response = await fetch(connection.url, { method: "HEAD", headers }); if (!response.ok) throw new RrError(`connection verification failed: HTTP ${response.status}`); }
