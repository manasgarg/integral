...
export interface Connection {
  name: string; kind: ConnectionKind; provider?: string; url?: string; auth: AuthMethod;
  methods?: string[]; pathPrefix?: string; header?: string; scheme?: string;
  authorizationUrl?: string; tokenUrl?: string; deviceAuthorizationUrl?: string;
  clientId?: string; scopes?: string[]; transport?: "streamable-http" | "sse";
}
...
const knownKeys = new Set(["name", "kind", "provider", "url", "auth", "methods", "path_prefix", "header", "scheme", "authorization_url", "token_url", "device_authorization_url", "client_id", "scopes", "transport"]);

function table(raw: unknown): Record<string, unknown> {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) throw new RrError("connection declaration must be a TOML table");
  return raw as Record<string, unknown>;
}
function requiredString(raw: unknown, key: string): string {
  if (typeof raw !== "string" || !raw.trim()) throw new RrError(`${key} is required`);
  return raw;
}
function optionalStrings(raw: unknown, key: string): string[] | undefined {
  if (raw === undefined) return undefined;
  if (!Array.isArray(raw) || raw.some((v) => typeof v !== "string" || !v)) throw new RrError(`${key} must be a list of strings`);
  return raw as string[];
}
function secureUrl(raw: unknown, key: string): string {
  const value = requiredString(raw, key);
  let parsed: URL;
  try { parsed = new URL(value); } catch { throw new RrError(`${key} must be a valid HTTP or HTTPS URL`); }
  if (!["http:", "https:"].includes(parsed.protocol)) throw new RrError(`${key} must use HTTP or HTTPS`);
  return parsed.toString();
}
function oauthUrl(raw: unknown, key: string): string {
  const value = secureUrl(raw, key); const url = new URL(value);
  if (url.protocol !== "https:" && !["localhost", "127.0.0.1", "::1"].includes(url.hostname)) throw new RrError(`${key} must use HTTPS unless it targets loopback`);
  return value;
}
...
export function validateConnection(raw: unknown, stem?: string): Connection {
  ...
  if (stem && stem !== name) throw new RrError(`connection file stem ${stem} does not match declared name ${name}`);
  const kind = requiredString(value.kind, "kind") as ConnectionKind;
  if (!["model", "http", "mcp"].includes(kind)) throw new RrError("kind must be model, http, or mcp");
  const provider = value.provider === undefined ? undefined : requiredString(value.provider, "provider");
  if (kind === "model" && !CATALOG.some((e) => e.kind === "model" && e.name === provider)) throw new RrError("provider must name a catalog model provider");
  const catalog = CATALOG.find((e) => e.name === provider);
  const auth = (value.auth ?? (catalog && "defaultAuth" in catalog ? catalog.defaultAuth : undefined)) as AuthMethod | undefined;
  if (!auth || !["oauth", "device-code", "key", "none"].includes(auth)) throw new RrError("auth must be oauth, device-code, key, or none");
  if (kind === "model" && auth === "none") throw new RrError("model connections do not support no authentication");
  const url = kind === "model" ? undefined : secureUrl(value.url, "url");
  const methods = optionalStrings(value.methods, "methods")?.map((m) => m.toUpperCase()) ?? (kind === "http" ? ["*"] : undefined);
  if (methods && methods.length === 0) throw new RrError("methods must not be empty");
  if (methods?.some((m) => m !== "*" && !/^[A-Z]+$/.test(m))) throw new RrError("methods must contain HTTP methods or *");
  let pathPrefix = value.path_prefix === undefined ? undefined : requiredString(value.path_prefix, "path_prefix");
  if (url && pathPrefix) {
    if (!pathPrefix.startsWith("/")) throw new RrError("path_prefix must start with /");
    const basePath = new URL(url).pathname;
    if (!pathPrefix.startsWith(basePath)) throw new RrError("path_prefix may only narrow the connection URL path");
  }
  ...
  if (auth === "key") {
    result.header = value.header === undefined ? "Authorization" : requiredString(value.header, "header");
    result.scheme = value.scheme === undefined ? "Bearer" : requiredString(value.scheme, "scheme");
    if (/\r|\n/.test(result.header + result.scheme)) throw new RrError("header and scheme must not contain control characters");
  }
  if (auth === "oauth" || auth === "device-code") {
    if (kind !== "model") {
      result.authorizationUrl = oauthUrl(value.authorization_url, "authorization_url");
      result.tokenUrl = oauthUrl(value.token_url, "token_url");
      result.clientId = requiredString(value.client_id, "client_id");
      if (auth === "device-code") result.deviceAuthorizationUrl = oauthUrl(value.device_authorization_url, "device_authorization_url");
      const scopes = optionalStrings(value.scopes, "scopes"); if (scopes) result.scopes = scopes;
    }
  }
  if (kind === "mcp") {
    const transport = value.transport ?? "streamable-http";
    if (transport !== "streamable-http" && transport !== "sse") throw new RrError("transport must be streamable-http or sse");
...
export function connectionToml(c: Connection): string {
  ...
  const scalar: [string, string | undefined][] = [["provider", c.provider], ["url", c.url], ["auth", c.auth], ["path_prefix", c.pathPrefix], ["header", c.header], ["scheme", c.scheme], ["authorization_url", c.authorizationUrl], ["token_url", c.tokenUrl], ["device_authorization_url", c.deviceAuthorizationUrl], ["client_id", c.clientId], ["transport", c.transport]];
  for (const [key, value] of scalar) if (value !== undefined && !rows.some((row) => row.startsWith(`${key} =`))) rows.push(`${key} = ${quote(value)}`);
  if (c.methods) rows.push(`methods = [${c.methods.map(quote).join(", ")}]`);
  if (c.scopes) rows.push(`scopes = [${c.scopes.map(quote).join(", ")}]`);
  return `${rows.join("\n")}\n`;
}

export async function loadConnections(paths: RrPaths): Promise<{ connections: Connection[]; errors: string[] }> {
  let files: string[];
  try { files = (await readdir(paths.connections)).filter((f) => f.endsWith(".toml")).sort(); }
  catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return { connections: [], errors: [] }; throw error; }
  const connections: Connection[] = [], errors: string[] = [], names = new Set<string>();
  for (const file of files) {
    try {
      const c = validateConnection(parse(await readFile(join(paths.connections, file), "utf8")), basename(file, ".toml"));
      if (names.has(c.name)) throw new RrError(`duplicate connection name: ${c.name}`);
      names.add(c.name); connections.push(c);
...
export async function removeCredential(paths: RrPaths, name: string): Promise<void> { await rm(join(paths.credentials, name), { force: true }); await bumpGeneration(paths); }
export async function removeConnection(paths: RrPaths, name: string): Promise<void> {
  const file = join(paths.connections, `${name}.toml`);
  try { await stat(file); } catch { throw new RrError(`connection not found: ${name}`); }
  await rm(file); await rm(join(paths.credentials, name), { force: true }); await bumpGeneration(paths);
}

export async function prepareStorage(paths: RrPaths): Promise<void> { await ensureDir(paths.connections); await ensureDir(paths.credentials); }
