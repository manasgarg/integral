...
interface Started { component: Component; stop: () => Promise<void>; unlock: () => Promise<void>; logger: Logger }
...
export async function startComponents(paths: RrPaths, config: EffectiveConfig, selected?: Component): Promise<void> {
  ...
  try {
    for (const component of components) {
      ...
      const logger = new Logger({ component, deploymentId: deploymentId(paths), level: config.logging.level, format: config.logging.format, secrets });
      ...
      catch (error) { logger.event("error", "component.start_failed", error instanceof Error ? error.message : String(error), { old_state: "starting", new_state: "failed" }); await unlock(); throw error; }
      const stop = () => runtime.stop(); started.push({ component, stop, unlock, logger });
