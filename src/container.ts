...
export async function writeMcpExtension(sessionHome: string, connections: Connection[]): Promise<void> {
  ...
  const declarations = connections.map((connection) => ({ name: connection.name.replace(/[^A-Za-z0-9_]/g, "_"), url: connection.url, auth: connection.auth !== "none", transport: connection.transport }));
...
export function ensureContainerImage(config: EffectiveConfig): void {
  ...
  if (config.runner.image === DEFAULT_PI_IMAGE) {
    ...
    const result = spawnSync("docker", ["build", "--pull", "--build-arg", `PI_VERSION=${PI_VERSION}`, "--tag", config.runner.image, "--file", dockerfile, root], { encoding: "utf8" });
