...
import { componentIdentity, deploymentId, readComponentState, verifyInternal } from "./state.ts";
import type { Logger } from "./logging.ts";
import { RrError } from "./errors.ts";

export interface ClientEvent { sequence: number; type: string; data: unknown }
export class Coordinator {
  readonly queue: DurableQueue; readonly conversation: ConversationStore; private readonly events = new EventEmitter(); private server: http.Server | undefined; private eventSequence = 0; private attached = 0; private token = "";
  ...
  async start(): Promise<http.Server> {
    ...
    await new Promise<void>((resolve, reject) => { server.once("error", reject); server.listen(this.config.server.coordinatorPort, "127.0.0.1", resolve); }); return server;
  }
  ...
  private async route(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      if (url.pathname === "/rr/health" && req.method === "GET") { json(res, 200, { component: "coordinator", deploymentId: deploymentId(this.paths), status: "ready" }); return; }
      ...
      if (queueMatch && req.method === "PATCH") { const body = await bodyJson(req); json(res, 200, await this.queue.edit(queueMatch[1]!, String(body.text ?? ""))); return; }
      if (queueMatch && req.method === "DELETE") { await this.queue.delete(queueMatch[1]!); res.writeHead(204).end(); return; }
      ...
      if (url.pathname === "/rr/status" && req.method === "GET") {
        ...
        json(res, 200, { gateway: gateway?.status ?? "unavailable", runner: runner?.status ?? "unavailable", container: runner?.status === "ready" ? "available" : "unavailable", session: this.conversation.snapshot().filter((e) => e.type === "session").at(-1)?.sessionId ?? null, provider: this.config.model.connection ?? "auto", queueDepth: queue.filter((m) => m.status === "queued").length, inFlight: queue.find((m) => m.status === "in-flight")?.id ?? null, attached: this.attached }); return;
      }
  ...
  private stream(req: IncomingMessage, res: ServerResponse): void {
    res.writeHead(200, { "content-type": "text/event-stream", "cache-control": "no-cache", connection: "keep-alive" }); this.attached++;
    ...
    req.once("close", () => { this.events.off("event", listener); this.attached--; });
  }
  async stop(): Promise<void> { const server = this.server; this.server = undefined; if (server) await new Promise<void>((resolve) => server.close(() => resolve())); }
}
