...
import { DEFAULT_PI_IMAGE, DEFAULT_PORTS, type Component } from "./constants.ts";
...
export interface EffectiveConfig {
  ...
  runner: {
    image: string; pullPolicy: PullPolicy; turnTimeoutSeconds: number;
    idleTimeoutSeconds: number; memoryMb: number; tmpfsMb: number;
  };
...
const allowed: Record<string, readonly string[]> = {
  ...
  runner: ["image", "pull_policy", "turn_timeout_seconds", "idle_timeout_seconds", "memory_mb", "tmpfs_mb"],
...
function int(value: unknown, key: string, positive = true): number {
  if (!Number.isInteger(value) || (positive ? Number(value) <= 0 : Number(value) < 0)) {
    throw new RrError(`${key} must be a ${positive ? "positive" : "non-negative"} integer`);
  }
  return Number(value);
}
...
function stringChoice<T extends string>(value: unknown, key: string, choices: readonly T[]): T {
...
function stringValue(value: unknown, key: string): string {
  if (typeof value !== "string" || !value.trim()) throw new RrError(`${key} must be a non-empty string`);
  return value;
}

export function validateMain(raw: unknown, file = "rr.toml"): Record<string, unknown> {
  const root = object(raw);
  const errors: string[] = [];
  for (const [section, value] of Object.entries(root)) {
    if (!allowed[section]) { errors.push(`${file}: unknown section [${section}]`); continue; }
    if (object(value) !== value) { errors.push(`${file}: [${section}] must be a table`); continue; }
    for (const key of Object.keys(value as object)) {
      if (!allowed[section]!.includes(key)) errors.push(`${file}: unknown option ${section}.${key}`);
      if (/credential|secret|token|api[_-]?key|password/i.test(key)) errors.push(`${file}: ${section}.${key}: credentials must be stored through rr connection add`);
    }
  }
...
function parseEnvPort(env: NodeJS.ProcessEnv, name: string): number | undefined {
  const raw = env[name]?.trim();
  if (!raw) return undefined;
  if (!/^\d+$/.test(raw)) throw new RrError(`${name} must be a decimal port from 1 through 65535`);
  return port(Number(raw), name);
}

export async function loadConfig(paths: RrPaths, env: NodeJS.ProcessEnv = process.env): Promise<EffectiveConfig> {
  const text = await readText(paths.mainConfig);
  let root: Record<string, unknown> = {};
  if (text !== undefined) {
    try { root = validateMain(parse(text), paths.mainConfig); }
    catch (error) { if (error instanceof RrError) throw error; throw new RrError(`${paths.mainConfig}: ${String(error)}`); }
  }
  ...
  const logging = {
    level: stringChoice(logLevelRaw, env.RR_LOG_LEVEL?.trim() ? "RR_LOG_LEVEL" : "logging.level", ["error", "warn", "info", "debug", "trace"]),
    format: stringChoice(logFormatRaw, env.RR_LOG_FORMAT?.trim() ? "RR_LOG_FORMAT" : "logging.format", ["text", "json"]),
  };
  const runner = {
    image: stringValue(val("runner", "image"), "runner.image"),
    pullPolicy: stringChoice(val("runner", "pull_policy"), "runner.pull_policy", ["always", "if-not-present", "never"]),
    turnTimeoutSeconds: int(val("runner", "turn_timeout_seconds"), "runner.turn_timeout_seconds"),
    idleTimeoutSeconds: int(val("runner", "idle_timeout_seconds"), "runner.idle_timeout_seconds"),
    memoryMb: int(val("runner", "memory_mb"), "runner.memory_mb"),
    tmpfsMb: int(val("runner", "tmpfs_mb"), "runner.tmpfs_mb"),
  };
  if (!/^[a-zA-Z0-9][a-zA-Z0-9._/:@-]*$/.test(runner.image)) throw new RrError("runner.image must be a valid OCI image reference");
  const conversation = {
    contextMaxMessages: int(val("conversation", "context_max_messages"), "conversation.context_max_messages", false),
    contextMaxChars: int(val("conversation", "context_max_chars"), "conversation.context_max_chars", false),
  };
  const modelRaw = object(root.model);
  const model: EffectiveConfig["model"] = {};
  if (modelRaw.connection !== undefined) model.connection = stringValue(modelRaw.connection, "model.connection");
  if (modelRaw.model !== undefined) model.model = stringValue(modelRaw.model, "model.model");
  ...
  for (const [component, envValue] of Object.entries(envPorts)) if (envValue !== undefined) sources[`server.${component}_port`] = "environment";
...
export async function initConfig(paths: RrPaths): Promise<void> {
  if (await readText(paths.mainConfig) !== undefined) throw new RrError(`${paths.mainConfig} already exists; refusing to overwrite it`);
