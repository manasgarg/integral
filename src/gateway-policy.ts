...
export function decideRequest(method: string, target: URL, headers: IncomingHttpHeaders, candidates: readonly CredentialedConnection[]): GatewayDecision {
  const match = candidates.find(({ connection }) => {
    ...
    return boundary.protocol === target.protocol && boundary.hostname.toLowerCase() === target.hostname.toLowerCase() && normalizePort(boundary) === normalizePort(target)
  ...
  for (const [key, value] of Object.entries(headers)) {
    if (value !== undefined && !["proxy-authorization", "proxy-connection"].includes(key.toLowerCase())) clean[key] = value;
  }
  if (match.connection.auth !== "none") {
    if (!match.credential) throw new RrError(`connection ${match.connection.name} has no usable credential; rotate or reconfigure it`, 403);
...
export function parseProxyAuthorization(value: string | undefined): string | undefined {
  if (!value?.startsWith("Basic ")) return undefined;
  try { const decoded = Buffer.from(value.slice(6), "base64").toString(); const separator = decoded.indexOf(":"); return separator >= 0 ? decoded.slice(separator + 1) : undefined; }
